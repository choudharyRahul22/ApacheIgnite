...
import com.blu.imdg.jdbc.CacheJdbcPersonStore;
...
public class CacheStoreSample {
	...
	private static final Object[] arr = new Object[LIMIT];
	

	/**
	 * This is an entry point of CacheStoreSample, the ignite configuration lies
	 * upon resources directory as example-ignite.xml.
	 *
	 * @param args
	...
	public static void main(String[] args) throws Exception {
		/*
		 * if(args.length <= 0 ){ LOGGER.
		 * error("Usages! java -jar .\\target\\cache-store-runnable.jar postgresql|mongodb"
		 * ); System.exit(0); } if(args[0].equalsIgnoreCase(POSTGRESQL)){
		 * jdbcStoreExample(); } else if (args[0].equalsIgnoreCase(MONGODB)){
		 * nosqlStore(); }
		 */
		
		// Create cache, write to cache and finally commit to db
		jdbcStoreExample();
		
	}

	private static void jdbcStoreExample() throws Exception {
		// let's make a dynamic cache on the fly which is distributed across all running
		// nodes.
		...
		configuration.setCacheStoreFactory(FactoryBuilder.factoryOf(CacheJdbcPersonStore.class));
		...
		try (Ignite ignite = Ignition.start(cfg)) {
			...
			int count = 10;
			try (IgniteCache<Long, Person> igniteCache = ignite.getOrCreateCache(configuration)) {
				try (Transaction tx = ignite.transactions().txStart(PESSIMISTIC, REPEATABLE_READ)) {
					// let us clear
					/*System.out.println("Loading Starting");
					igniteCache.loadCache(null, 4);
					System.out.println("Loding Ends");*/
					
					System.out.println("Loading Starting");
					//CacheJdbcPersonStore store = new CacheJdbcPersonStore();
					igniteCache.loadCache((Long, Person) -> true, Integer.MAX_VALUE); 
					System.out.println("Loding Ends");
					
					System.out.println(igniteCache.get((long) 1.0));

					/*for (int i = 1; i <= count; i++)
						igniteCache.put("_" + i, new Post("_" + i, "title-" + i, "description-" + i,
								LocalDate.now().plus(i, ChronoUnit.DAYS), "author-" + i));

					System.out.println("Transaction Commiting Start- 1");
					tx.commit();
					System.out.println("Transaction Commiting End- 1");
					...
					System.out.println("Transaction Commiting Start- 2");
					tx.commit();
					System.out.println("Transaction Commiting End- 2");*/
				}
			}
		...
		try (Ignite ignite = Ignition.start(cfg)) {
			...
			try (IgniteCache<String, MongoPost> igniteCache = ignite.getOrCreateCache(configuration)) {
				try (Transaction tx = ignite.transactions().txStart(PESSIMISTIC, REPEATABLE_READ)) {
					...
					for (int i = 1; i <= count; i++)
						igniteCache.put("_" + i, new MongoPost("_" + i, "title-" + i, "description-" + i,
								LocalDate.now().plus(i, ChronoUnit.DAYS), "author-" + i));
